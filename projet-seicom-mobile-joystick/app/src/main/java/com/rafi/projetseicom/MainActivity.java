...
public class MainActivity extends Activity implements RadioGroup.OnCheckedChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        angleTextView = (TextView) findViewById(R.id.angletextViewLab);
        ...
        joystick.setOnJoystickMoveListener(new JoystickView.OnJoystickMoveListener() {
            ...
            public void onValueChanged(int angle, int power, int direction) {
                if (joystickEnabled) {
                    // Joystick is enabled, update its behavior
                    angleTextView.setText(" " + String.valueOf(angle) + "°");
                    if (mService != null) {
                        // Set the sign bit (bit 7) based on the angle value
                        byte signBit = (byte) ((angle < 0) ? 0x80 : 0x00);

                        // Get the absolute value of the angle
                        int absAngle = Math.abs(angle);

                        // Split the absolute angle value into two bytes
                        byte angleHighByte = (byte) ((absAngle >> 8) & 0xFF);
                        byte angleLowByte = (byte) (absAngle & 0xFF);

                        // Combine the sign bit and the angle bytes into a single byte array
                        byte[] message = new byte[] { (byte) (signBit | angleHighByte), angleLowByte };

                        // Send the message to the BLE device
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                byte[] message = new byte[] {0, (byte) progress};
                mService.writeRXCharacteristic(message);
            }
